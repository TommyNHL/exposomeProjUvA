...
function strat_split(leverage=h; limits = limits)
    ...
    X_retain, X_remove, y_retain, y_remove = train_test_split(collect(1:length(leverage)), leverage, test_size = 0.30, random_state = 42, stratify = bin)
...
inputDfOnlyTN[X_trainIdx, "GROUP"] .= "train"
inputDfOnlyTN[X_testIdx, "GROUP"] .= "test"
count = 1
for i in X_trainIdx
    inputDfOnlyTN[i, "Leverage"] = train_lev[count]
    count += 1
end
count = 1
for i in X_testIdx
    inputDfOnlyTN[i, "Leverage"] = test_lev[count]
    count += 1
end
