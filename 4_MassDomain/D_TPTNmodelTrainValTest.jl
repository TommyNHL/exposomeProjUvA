...
function optimRandomForestRegressor(df_train)
    ...
    while itr < 66
        ...
        println("## Regression ##")
        reg = RandomForestClassifier(n_estimators=t, min_samples_leaf=l, max_features=MaxFeat, n_jobs=-1, oob_score =true, random_state=42, class_weight=Dict(0=>0.529, 1=>9.097))
        println("## fit ##")
