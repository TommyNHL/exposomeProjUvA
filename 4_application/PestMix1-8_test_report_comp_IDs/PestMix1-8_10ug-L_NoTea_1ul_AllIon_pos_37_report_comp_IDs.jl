...
inputTestDB_withDeltaRiTPTN = CSV.read("F:\\UvA\\F\\UvA\\app\\PestMix1-8_10ug-L_NoTea_1ul_AllIon_pos_37_report_comp_IDs_withDeltaRIandPredictedTPTN_KNN.csv", DataFrame)
    #
    maxAE_val, MSE_val, RMSE_val = errorDetermination(inputTestDB_withDeltaRiTPTN[:, end-2], inputTestDB_withDeltaRiTPTN[:, end])  # 1, 0.3361921097770154, 0.5798207565938075
    pTP_test = predict_proba(model, Matrix(inputTestDB_withDeltaRiTPTN[:, vcat(5, 7,9, 13,14, 17)]))  # 583 × 2 Matrix
    recall_test = recall_score(Vector(inputTestDB_withDeltaRiTPTN[:, end-2]), predict(model, Matrix(inputTestDB_withDeltaRiTPTN[:, vcat(5, 7,9, 13,14, 17)])))  # 0.5605095541401274
