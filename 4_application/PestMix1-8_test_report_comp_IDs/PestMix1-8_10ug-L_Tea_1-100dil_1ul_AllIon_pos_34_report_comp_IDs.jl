...
inputTestDB_withDeltaRiTPTN = CSV.read("F:\\UvA\\F\\UvA\\app\\PestMix1-8_10ug-L_Tea_1-100dil_1ul_AllIon_pos_34_report_comp_IDs_withDeltaRIandPredictedTPTN_KNN.csv", DataFrame)
    #
    maxAE_val, MSE_val, RMSE_val = errorDetermination(inputTestDB_withDeltaRiTPTN[:, end-2], inputTestDB_withDeltaRiTPTN[:, end])  # 1, 0.4018867924528302, 0.6339454175659213
    pTP_test = predict_proba(model, Matrix(inputTestDB_withDeltaRiTPTN[:, vcat(5, 7,9, 13,14, 17)]))  # 1060 × 2 Matrix
    recall_test = recall_score(Vector(inputTestDB_withDeltaRiTPTN[:, end-2]), predict(model, Matrix(inputTestDB_withDeltaRiTPTN[:, vcat(5, 7,9, 13,14, 17)])))  # 0.5839793281653747
